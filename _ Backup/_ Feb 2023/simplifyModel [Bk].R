...
if( simplifyType == "negativeRelativeImportance" ) {
  ...
  keptVariables <- names(subset(rasterLayers, which(names(rasterLayers) %in% relativeContributionDF[which(relativeContributionDF$relImportance > keepPredictorsThreshold),"Predictor"]) ))
...
modelReduction <- list(model=modelFull$model,plot=lossFunctionPlot,keptVariables=keptVariables,removedVariables=removedVariables)
