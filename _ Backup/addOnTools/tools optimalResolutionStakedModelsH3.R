...
dataFolderRecords <- "../../../Data/Distribution Models/Global distribution of seagrasses/Data/Records/"
dataFolderRasters <- "/Volumes/StingRay/Dropbox/Manuscripts/Future biodiversity of seagrass forests in a changing climate/Results/Maps/"

## -------------------------------------------------------------------------------

occurrenceRecords <- paste0(dataFolderRecords,"/finalDatasetSeagrassesReduced.csv")
occurrenceRecords <- read.csv(occurrenceRecords)
occurrenceRecords <- occurrenceRecords[complete.cases(occurrenceRecords),]
...
rasters <- list.files(dataFolderRasters,full.names = TRUE,pattern="RData")
rasters
file <- 33
rasters[file]
rasterMap <- loadRData(rasters[file])
...
for(i in 1:length(regions)) {
  ...
  aggrement[i,"region"] <- r
  aggrement[i,"observed"] <- length(unique(occurrenceRecordsIn.r$name))
  aggrement[i,"predicted"] <- unique(occurrenceRecordsIn.r$Value)[!is.na(unique(occurrenceRecordsIn.r$Value))]
...
p3 <- ggplot(aggrement, aes(x=observed, y=predicted)) +
  ...
  annotate(geom="text", x=3, y=max(aggrement$predicted), col="black", 
           label="Correlation: 0.74", parse=T, fontface=1, size=4) +  theme_minimal()
p3

pdf(file=gsub(".Rdata","Agreement.Rdata",rasters[file]),width=8,height=7,useDingbats=FALSE)
p3
dev.off()
